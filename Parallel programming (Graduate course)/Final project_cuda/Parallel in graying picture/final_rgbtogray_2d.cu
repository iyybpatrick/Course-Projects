...
__global__ void gray_f(unsigned char *pBmpBuf,int *lineByte,int *height,int *width)
{
	...
	j=blockIdx.y*blockDim.y+threadIdx.y;
	 int wid_data=(*width)/256+1;
	 int hei_data=(*height)/256+1;
			for(j=j*hei_data;(j<(j+hei_data))&&(j<(*height));j++)
				for(i=i*wid_data;(i<(i+wid_data))&&(i<(*width));i++){
					blue=*(pBmpBuf+i*(*lineByte)+j*3+0);
					green=*(pBmpBuf+i*(*lineByte)+j*3+1);//绿色分量
					red=*(pBmpBuf+i*(*lineByte)+j*3+2);//红色分量
					gray=(30*red+59*green+11*blue)/100;//计算灰度值
					*(pBmpBuf+i*(*lineByte)+j*3+0)=(BYTE)gray; //将原来蓝色分量用灰度值替代
					*(pBmpBuf+i*(*lineByte)+j*3+1)=(BYTE)gray; //将原来绿色分量用灰度值替代
					*(pBmpBuf+i*(*lineByte)+j*3+2)=(BYTE)gray; //将原来红色分量用灰度值替代
		}
}
int main(){
	...
	char readPath[]="6144_2048.bmp"; //读入图像名称
	...
	if(biBitCount==8){}
			...
			//	printf("185\n");
				gray_f<<<dimGrid,dimBlock>>>(b,a,c,d);
