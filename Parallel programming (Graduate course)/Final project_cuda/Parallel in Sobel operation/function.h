...
int printBmpPixelData(char *path)
{
    BITMAPINFOHEADER bmih;
    int ret;
    int width;
    ...
    if(ret==-1)
    {
        printf("Error(printBmpPixelData): the path of the image is invalid.\n");
        return -1;
    }
    ...
    if(ret==-1)
    {
        printf("Error(printBmpPixelData): can not read the Info Header.\n");
        return -1;
    }
    ...
    img=(BYTE*)malloc(height*lineBytes*sizeof(BYTE));
    if(img==0)
    {
        printf("Error(printBmpPixelData): can not allocate memory for the image.\n");
        return -1;
    }

    ret=readBmpPixelData(path,img);
    if(ret==-1)
    {
        printf("Error(printBmpPixelData): can not read the pixel data of the image.\n");
        return -1;
    }
    ...
    else if(bitCount==24)
    {
        for(i=0;i<height;i++)
        {
            for(j=0;j<width*3;j++)
            {
                b=*(img+lineBytes*(height-1-i)+j);
                j++;
                g=*(img+lineBytes*(height-1-i)+j);
                j++;
                r=*(img+lineBytes*(height-1-i)+j);
    ...
    {
        printf("Error(printBmpPixelData): only supported 8 or 24 bits.\n");
        free(img);
        return -1;
    }
...
int printMenu()
{
    ...
    printf("1. Read File Header         \t\n");
    printf("2. Read Info Header         \t\n");
    printf("3. Read Pixel Data          \t\n");
    printf("4. Print File Header        \t\n");
    printf("5. Print Info Header        \t\n");
    printf("6. Print Pixel Data         \t\n");
    printf("7. Save As the image        \t\n");
    printf("0. Exit                 \t\n");
    scanf("%d",&ret);
    if(ret>=0 && ret<=7)
    {
